...
def _estimate_fair_rent(l: Dict[str, Any]) -> Dict[str, Any]:
    # Simple heuristic: base by city + area + distance + elevator + decoration
    ...
    # Factor contributions for UI waterfall usage
    factors = [
...
async def dashboard_summary() -> Dict[str, Any]:
    ...
    for l in latest:
        ...
        # very rough risk: long commute + far subway + no elevator high floor
        risk_score = 0
...
async def assessments_estimate(payload: Dict[str, Any]) -> Dict[str, Any]:
    # payload from Estimate.vue is already flattened, reuse same estimator
    l = {
...
async def assessments_risk(payload: Dict[str, Any]) -> Dict[str, Any]:
    # Expected fields: noise/mold/poor_light/old_appliances/sublease_risk/contract_unfair (0/1/2)
    weights = {
        "noise": 25,
        "mold": 35,
        "poor_light": 25,
...
async def commute_analyze(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for minutes in [20, 30, 40, 50, 60]:
        ...
        rent_saving = max(0.0, (minutes - 20) * 60)  # demo heuristic
